import { c as createRule, a as isCommentToken, r as isSemicolonToken, i as isTokenOnSameLine, e as isOpeningParenToken, s as isQuestionDotToken, f as isClosingParenToken, l as isNotOpeningParenToken, t as isOpeningBraceToken, j as isOpeningBracketToken, n as isClosingBracketToken, u as isEqToken, v as isColonToken, m as isNotClosingParenToken, w as isClosingBraceToken, x as isNotSemicolonToken, y as skipChainExpression, S as STATEMENT_LIST_PARENTS, z as createGlobalLinebreakMatcher } from '../utils.js';
  ...
  create(context, optionsWithDefaults) {
    ...
    function addBlocklessNodeIndent(node) {
      if (node.type !== "BlockStatement") {
        ...
        while (isOpeningParenToken(sourceCode.getTokenBefore(firstBodyToken)) && isClosingParenToken(sourceCode.getTokenAfter(lastBodyToken))) {
    ...
    const baseOffsetListeners = {
      ...
      "ClassDeclaration, ClassExpression": function(node) {
        if (!node.superClass)
          return;
      ...
      ConditionalExpression(node) {
        ...
        if (!options.flatTernaryExpressions || !isTokenOnSameLine(node.test, node.consequent) || isOnFirstLineOfStatement(firstToken, node)) {
      ...
      ExportNamedDeclaration(node) {
        if (node.declaration === null) {
          const closingCurly = node.source ? sourceCode.getTokenBefore(node.source, isClosingBraceToken) : sourceCode.getLastToken(node, isClosingBraceToken);
      ...
      ":matches(DoWhileStatement, ForStatement, ForInStatement, ForOfStatement, IfStatement, WhileStatement, WithStatement):exit": function(node) {
        ...
        for (const nodeToCheck of nodesToCheck) {
          ...
          if (isSemicolonToken(lastToken)) {
            ...
            if (!isTokenOnSameLine(tokenBeforeLast, lastToken) && tokenAfterLast && isTokenOnSameLine(lastToken, tokenAfterLast)) {
              offsets.setDesiredOffset(
                lastToken,
                sourceCode.getFirstToken(node),
                0
      ...
      NewExpression(node) {
        if (node.arguments.length > 0 || isClosingParenToken(sourceCode.getLastToken(node)) && isOpeningParenToken(sourceCode.getLastToken(node, 1))) {
    ...
    function TSPropertySignatureToProperty(node, type = AST_NODE_TYPES.Property) {
      const base = {
...
${indentStr}${p1}`);
      ...
      TSMappedType(node) {
        ...
        const squareBracketStart = sourceCode2.getTokenBefore(
