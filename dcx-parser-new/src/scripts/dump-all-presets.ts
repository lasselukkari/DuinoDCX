...
function loadPage(pageNumber: number): Uint8Array {
...
function extractSysexMessages(bytes: Uint8Array): Uint8Array[] {
        ...
        while (offset < bufferToParse.length - 4) {
          ...
          if (
            ...
            try {
              ...
              console.log(
                `    Parsed preset: "${presetProto.header.presetName.trim()}"`,
              );
              ...
              console.error(`    Error parsing preset at offset ${offset} in Page ${i}:`, (error as Error).message);
