...
async function main() {
  ...
  port.on('open', async () => {
    try {
      ...
      const ebPart0 = await dumpData(
        port,
        buildEditBufferRequest(0),
        'Edit Buffer Part 0',
      );
      await sleep(200);
      const ebPart1 = await dumpData(
        port,
        buildEditBufferRequest(1),
        'Edit Buffer Part 1',
      );
      ...
      const preset00 = await dumpData(
        port,
        buildPageDumpRequest(0),
        'Preset 1 (Index 0)',
      );
      ...
      const preset01 = await dumpData(
        port,
        buildPageDumpRequest(1),
        'Preset 2 (Index 1)',
      );
      ...
      const preset59 = await dumpData(
        port,
        buildPageDumpRequest(59),
        'Preset 60 (Index 59)',
      );
