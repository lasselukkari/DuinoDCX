...
function main() {
  ...
  const data = fs.readFileSync(
    path.join(
      process.cwd(),
      'test-data/verification/preset_00_calibration.bin',
    ),
  );
  ...
  for (let prefix = 30; prefix <= 50; prefix++) {
    ...
    if (eqStart + 10 > data.length) continue;
    ...
    if (freq === 180 && q === 20 && gain === 150) {
      ...
      console.log(
        `Values: Freq=${freq}, Q=${q}, Gain=${gain}, Type=${type}, Shelf=${shelf}`,
      );
      ...
      if (end8 === 124) console.log(`-> FITS PERFECTLY with 8 bands`);
      if (end9 === 124) console.log(`-> FITS PERFECTLY with 9 bands`);
    }
  }
}
