 ...
 */
import { SYSEX_START, SYSEX_END, VENDOR_ID, COMMAND_BYTE_INDEX, RSP_SEARCH, RSP_DUMP, RSP_ACK, CMD_DUMP_REQUEST, CMD_DIRECT, HEADER_SIZE, } from '../constants/protocol.js';
 ...
 */
export function parseDevices(message) {
 ...
 */
function parseSearchResponse(message, deviceId) {
 ...
 */
function parseDumpResponse(message, deviceId) {
 ...
 */
function parsePageRequest(message, deviceId) {
    return {
        type: 'pageRequest',
        deviceId,
        page: message[9],
 ...
 */
function parseDirectCommand(message, deviceId) {
 ...
 */
export function isValidSysex(message) {
 ...
 */
export function extractSysexMessages(buffer) {
    const messages = [];
    let start = -1;
