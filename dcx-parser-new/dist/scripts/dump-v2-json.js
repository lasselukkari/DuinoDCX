...
function extractSysExData(packet) {
    if (packet.length < 15) {
...
function main() {
    try {
        ...
        const jsonOutput = JSON.stringify(state, (_key, value) => {
            if (value instanceof Uint8Array) {
                return `[Uint8Array ${value.length} bytes]`; // Simplify binary data
            }
            return value;
        }, 2);
        const outputPath = path.join(process.cwd(), 'test-data', 'calibration', 'edit_buffer_v2_output.json');
...
main();
//# sourceMappingURL=dump-v2-json.js.map