...
function extractAndDecode(sysexStream) {
    const messages = [];
    let start = -1;
    for (let i = 0; i < sysexStream.length; i++) {
        if (sysexStream[i] === 0xf0)
            start = i;
...
function main() {
    ...
    const inputA = parsed.inputs.A;
    if (inputA?.gain) {
        console.log(`Input A Gain: raw=${inputA.gain.raw}, value=${inputA.gain.value}`);
    ...
    if (inputA?.dynamicEqualizerGain) {
        console.log(`Input A DynEQ Gain: raw=${inputA.dynamicEqualizerGain.raw}, value=${inputA.dynamicEqualizerGain.value}`);
    }
    if (inputA?.eq1) {
        console.log(`Input A EQ1 Freq: raw=${inputA.eq1.frequency?.raw}, value=${inputA.eq1.frequency?.value}`);
        console.log(`Input A EQ1 Q: raw=${inputA.eq1.q?.raw}, value=${inputA.eq1.q?.value}`);
        console.log(`Input A EQ1 Gain: raw=${inputA.eq1.gain?.raw}, value=${inputA.eq1.gain?.value}`);
    }
}
main();
//# sourceMappingURL=test-preset.js.map