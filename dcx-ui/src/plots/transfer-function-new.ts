...
class TransferFunction {
    ...
    constructor(
        ...
        private readonly sampleRate: number = 96000,
    ) {
    ...
    parametricEQ(centerFrequency: number, gainDb: number, qualityFactor: number): void {
        const omega = (2 * pi * centerFrequency) / this.sampleRate;
        const sinOmega = Math.sin(omega);
        const cosOmega = Math.cos(omega);
        const alpha = sinOmega / (2 * qualityFactor);
        const gainAmplitude = Math.pow(10, gainDb / 40);
    ...
    lowShelving(
        ...
        const b0 = gainAmplitude * (gainAmplitude + 1 - (gainAmplitude - 1) * cosOmega + beta);
        const b1 = 2 * gainAmplitude * (gainAmplitude - 1 - (gainAmplitude + 1) * cosOmega);
        const b2 = gainAmplitude * (gainAmplitude + 1 - (gainAmplitude - 1) * cosOmega - beta);
    ...
    applyCrossover(type: string, crossoverFrequency: number, isHighpass: boolean): void {
        const apply2nd = isHighpass
            ? (f: number, q: number) => this.highPass2ndOrder(f, q)
            : (f: number, q: number) => this.lowPass2ndOrder(f, q);

        const apply1st = isHighpass
            ? (f: number) => this.highPass1stOrder(f)
            : (f: number) => this.lowPass1stOrder(f);
        ...
        switch (type.toLowerCase()) {
            case 'but6':
                ...
                break;
            case 'but12':
                apply2nd(crossoverFrequency, 0.707107);
                break;
            case 'but18':
                apply1st(crossoverFrequency);
                apply2nd(crossoverFrequency, 1.0);
                break;
            case 'but24':
    ...
    getMagnitude(): number[] {
        return this.transferFunction.map((point) => 20 * log10(abs(point) as any as number));
    }

    getAngle(): number[] {
        return this.transferFunction.map((point) => (point.toPolar().phi * 180) / pi);
    }
