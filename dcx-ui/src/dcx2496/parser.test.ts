...
describe('parseDirectCommand', () => {
    it('parses a mute command correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20, // header
            0x01,                                       // count = 1
            0x01, 0x03, 0x00, 0x01,                     // channel=1, param=3, hi=0, lo=1
            0xF7,                                       // terminator
        ]);
    ...
    it('parses a gain command correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x01,                                       // count = 1
            0x05, 0x02, 0x00, 0x78,                     // channel=5, param=2, hi=0, lo=120
            0xF7,
        ]);
        ...
        expect(deltas[0].value).toBeCloseTo(-3.0, 1);
    });
    ...
    it('parses multiple commands correctly', () => {
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x02,                                       // count = 2
            0x01, 0x03, 0x00, 0x01,                     // mute input A
            0x02, 0x03, 0x00, 0x00,                     // unmute input B
            0xF7,
        ]);
    ...
    it('parses setup command correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x01,
            0x00, 0x0B, 0x00, 0x2D,                     // channel=0, param=11, hi=0, lo=45
            0xF7,
        ]);
    ...
    it('parses EQ command correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x01,
            0x01, 0x14, 0x00, 0x32,                     // channel=1, param=20 (0x14), hi=0, lo=50
            0xF7,
        ]);
    ...
    it('parses output-only command correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x01,
            0x05, 0x49, 0x00, 0x01,                     // channel=5, param=73 (0x49), hi=0, lo=1
            0xF7,
        ]);
    ...
    it('handles high byte values correctly', () => {
        ...
        const buffer = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x20,
            0x01,
            0x05, 0x05, 0x27, 0x68,                     // channel=5, param=5 (longDelay), hi=39, lo=104
            0xF7,
        ]);
...
describe('decode7to8 / encode7to8', () => {
    it('decodes a simple 8-byte group correctly', () => {
        ...
        const encoded = new Uint8Array([
            0x05, 0x42, 0x00, 0x7F, 0x7F, 0x01, 0x00, // 7 data bytes
            0b00010001 // highBits: bit0=1 (for 0x85), bit4=1 (for 0xFF)
        ]);
    ...
    it('encodes data correctly', () => {
        ...
        expect(encoded[7]).toBe(0b00010001); // highBits
    });

    it('roundtrips correctly', () => {
        // Test that encode->decode produces original data
        const original = new Uint8Array([
            0x00, 0x7F, 0x80, 0xFF, 0x55, 0xAA, 0x01
        ]);
    ...
    it('handles multiple groups', () => {
        ...
        const original = new Uint8Array([
            0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Group 1
            0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Group 2
        ]);
...
describe('parseDumpResponse', () => {
    it('extracts part number and decodes payload', () => {
        ...
        const header = new Uint8Array([
            0xF0, 0x00, 0x20, 0x32, 0x00, 0x0E, 0x10, // bytes 0-6
            0x01, 0x00, 0x00, 0x00, 0x00,             // bytes 7-11
            0x00                                       // byte 12 = part 0
        ]);

        // Encoded payload: [0x05, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0b00000001]
        // Decodes to: [0x85, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00]
        const payload = new Uint8Array([0x05, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0b00000001]);
    ...
    it('handles part 1 correctly', () => {
        ...
        const payload = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        const terminator = new Uint8Array([0xF7]);
