...
import { buildPingCommand, buildPageDumpRequest, buildEditBufferRequest, buildRecallCommand, buildStoreCommand, buildDirectCommand, buildDataPacket, } from '../commands/builders.js';
import { calculateChecksum } from './checksum.js';
describe('sysex', () => {
    describe('buildPingCommand', () => {
        it('should build correct ping command', () => {
            const cmd = buildPingCommand(0x00);
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x40, 0xf7]));
        });
    });
    describe('buildPageDumpRequest', () => {
        it('should build correct page dump request', () => {
            const cmd = buildPageDumpRequest(0x05, 0x01);
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x01, 0x0e, 0x50, 0x00, 0x00, 0x05, 0xf7]));
        });
    ...
    describe('buildEditBufferRequest', () => {
        it('should build correct edit buffer request for part 0', () => {
            ...
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x50, 0x01, 0x00, 0x00, 0xf7]));
        });
        it('should build correct edit buffer request for part 1', () => {
            const cmd = buildEditBufferRequest(1, 0x00);
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x50, 0x01, 0x00, 0x01, 0xf7]));
        });
    });
    describe('buildRecallCommand', () => {
        it('should build correct recall command', () => {
            const cmd = buildRecallCommand(10, 0x00);
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x52, 10, 0xf7]));
        });
    });
    describe('buildStoreCommand', () => {
        it('should build correct store command', () => {
            const cmd = buildStoreCommand(60, 0x00);
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x53, 60, 0xf7]));
        });
    });
    describe('buildDirectCommand', () => {
        it('should build correct direct command for single parameter', () => {
            const cmd = buildDirectCommand([{ channel: 0, param: 1, value: 300 }], 0x00);
            // F0 00 20 32 00 0E 20 01 00 01 02 2C F7
            // 300 = 2 * 128 + 44
            // Hi = 2, Lo = 44 (0x2C)
            expect(cmd).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x20, 0x01, 0x00, 0x01, 0x02, 0x2c, 0xf7]));
        });
    });
    describe('isValidSysex', () => {
        it('should return true for valid message', () => {
            const msg = new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x40, 0xf7]);
            expect(isValidSysex(msg)).toBe(true);
        });
        it('should return false for too short message', () => {
            const msg = new Uint8Array([0xf0, 0xf7]);
            expect(isValidSysex(msg)).toBe(false);
        });
        it('should return false for invalid vendor ID', () => {
            const msg = new Uint8Array([0xf0, 0x00, 0x00, 0x00, 0x00, 0x0e, 0x40, 0xf7]);
            expect(isValidSysex(msg)).toBe(false);
        });
    });
    describe('extractSysexMessages', () => {
        it('should extract multiple messages', () => {
            const data = new Uint8Array([
                ...
                0xf0, 0x00, 0x20, 0x32, 0x01, 0x0e, 0x40, 0xf7,
            ]);
    ...
    describe('parseMessage', () => {
        it('should parse search response', () => {
            const msg = new Uint8Array([
                0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x00, 0x01, 0x01,
                0x44, 0x43, 0x58, 0x32, 0x34, 0x39, 0x36, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
                0xf7
            ]);
        ...
        it('should parse ACK response', () => {
            const msg = new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x52, 0x01, 0x02, 0xf7]);
            const parsed = parseMessage(msg);
        ...
        it('should parse page request', () => {
            const msg = new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x50, 0x00, 0x00, 0x05, 0xf7]);
            const parsed = parseMessage(msg);
        ...
        it('should parse direct parameter command', () => {
            const msg = new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x20, 0x01, 0x00, 0x01, 0x02, 0x2c, 0xf7]);
            const parsed = parseMessage(msg);
            expect(parsed?.type).toBe('direct');
            if (parsed?.type === 'direct') {
                expect(parsed.parameters).toHaveLength(1);
                expect(parsed.parameters[0]).toEqual({ channel: 0, param: 1, value: 300 });
            }
        });
    });
    describe('buildDataPacket', () => {
        it('should build correct data packet with checksum', () => {
            ...
            expect(packet.slice(0, 7)).toEqual(new Uint8Array([0xf0, 0x00, 0x20, 0x32, 0x00, 0x0e, 0x10]));
            expect(packet.slice(7, 13)).toEqual(new Uint8Array([0x00, 0x01, 0x00, 0x0c, 0x00, 0x05]));
            expect(packet.at(-2)).toBe(expectedChecksum);
            expect(packet.at(-1)).toBe(SYSEX_END);
        });
    });
});
//# sourceMappingURL=sysex.test.js.map