...
function formatInputChannel(options) {
    const { presetWords, inputName, inputGainOffset, inputMuteOffset, inputDelayOnOffset, sectionStart, } = options;
    ...
    const equalizerBands = Array.from({ length: 9 }, (_, i) => {
        ...
        return (`  Equalizer Band ${i + 1}: ${lookupFreq(presetWords[off])}, Q=${lookupQ(presetWords[off + 1])}, ` +
...
function formatPresetDump(presetWords, slotIndex, name) {
    ...
    const inputLines = inputNames.flatMap((inputName, chIdx) => formatInputChannel({
        presetWords,
        inputName,
    ...
    const outputLines = Array.from({ length: 6 }, (_, outIdx) => formatOutputChannel(presetWords, outIdx)).flat();
    return [
        '='.repeat(70),
...
describe('preset-parser', () => {
    ...
    describe('TypeScript vs Python verification', () => {
        ...
        it('should match Python formatted output', () => {
            ...
            const tsOutput = formatPresetDump(firstPreset.words, firstPreset.index, firstPreset.name);
            try {
...
"`;
                ...
                if (mismatches.length > 0) {
                    console.log(`Mismatches found (${mismatches.length} of ${tsLines.length} lines):`);
