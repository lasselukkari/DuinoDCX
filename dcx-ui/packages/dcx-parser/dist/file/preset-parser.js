 ...
 */
function parseInputNames(words) {
    const names = [];
 ...
 */
export function parsePresetWords(words) {
 ...
 */
export function parseDcxFileToStates(data) {
    // Verify magic
    if (data[0] !== 0x58 || // 'X'
        data[1] !== 0x53 || // 'S'
        data[2] !== 0x4e || // 'N'
 ...
 */
export function parseMemoryPages(pages) {
    ...
    if (combined[7] !== 0x58 || // 'X'
        combined[8] !== 0x53 || // 'S'
        combined[9] !== 0x4e || // 'N'
        combined[10] !== 0x50 // 'P'
    ) {
        throw new Error('Invalid memory dump: XSNP signature not found at expected offset');
    }
    ...
    const firstIndex = combined[indexOffset] +
