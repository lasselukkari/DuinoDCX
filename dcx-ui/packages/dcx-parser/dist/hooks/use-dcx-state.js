...
import { buildEditBufferRequest, buildParamChangeCommand } from '../commands/builders.js';
...
export function useDcxState(connection) {
     ...
     */
    const setSetup = useCallback(async (key, value) => {
        if (!connection)
            return;
     ...
     */
    const setChannel = useCallback(async (group, id, key, value) => {
        if (!connection)
            return;
     ...
     */
    const setEqualizer = useCallback(async (group, channelId, band, key, value) => {
        if (!connection)
            return;
        const target = { kind: 'equalizer', group, channelId, band, key };
        const cmd = buildParamChangeCommand(target, value);
        if (cmd) {
            await connection.send(cmd);
            // Optimistic update
