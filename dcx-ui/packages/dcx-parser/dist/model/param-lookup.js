 ...
 * - All lookups are O(1)
 */
import { setupParameters, channelParameters, outputOnlyParameters, equalizerParameters, } from './parameter-mappings.js';
// ============================================================================
// Constants
// ============================================================================
...
function buildWordLookup() {
    ...
    for (const parameter of channelParameters) {
        for (let i = 0; i < parameter.channels.length; i++) {
            const loc = parameter.channels[i];
            if (!loc || loc.wordOffset === undefined)
                continue;
            const isInput = i < 4;
            const channelId = CHANNEL_IDS[i];
            const def = {
        ...
        for (let band = 0; band < EQUALIZER_BANDS; band++) {
            for (const [i, parameter] of equalizerParameters.entries()) {
                ...
                const def = {
                    ...
                    min: parameter.min,
                    step: parameter.step,
                    target: { kind: 'equalizer', group, id: channelId, band: band + 1 },
                };
                def.target.channelId = def.target.id;
                lookup.set(makeDirectKey(ch, parameterNumber), def);
            }
...
export function applyToState(state, def, value) {
    ...
    switch (target.kind) {
        ...
        case 'equalizer': {
            state[target.group][target.channelId].equalizers[String(target.band)][key] = value;
            break;
        }
    }
}
