...
export function createEmptyEqualizer(): Equalizer {
  return {
    equalizerType: '',
    equalizerFrequency: '',
    equalizerGain: 0,
...
/**
 * Convert string to camelCase (e.g. "Input Sum Type" -> "inputSumType").
 */
export function camelize(string_: string): string {
  return string_
    .replaceAll(/\s+(.)/g, (_m, chr) => chr.toUpperCase())
    .replaceAll(/\s/g, '')
    .replace(/^(.)/, (_m, chr) => chr.toLowerCase());
}
