...
function lookupFreq(idx: number): string {
  const freq = constants.logFrequencyScale[idx];
  return freq ? `${freq} Hz` : `index ${idx}`;
}

function lookupQ(idx: number): string {
  return constants.EQ_Q_VALUES[idx] ?? `index ${idx}`;
}

function lookupEqType(idx: number): string {
  return constants.EQ_TYPES[idx] ?? `type ${idx}`;
}

function lookupEqSlope(idx: number): string {
  return constants.EQ_SHELVING_SLOPES[idx] ?? `slope ${idx}`;
}

function lookupXoverFilter(idx: number): string {
  return (constants.crossoverFilters[idx] ?? `filter ${idx}`).toUpperCase();
}

function lookupSource(idx: number): string {
  return constants.outputSources[idx] ?? `source ${idx}`;
}

function lookupPolarity(idx: number): string {
  return constants.POLARITIES[idx] ?? `polarity ${idx}`;
}

function lookupOutputConfig(idx: number): string {
  return (constants.OUTPUT_CONFIGS[idx] ?? `config ${idx}`).toUpperCase();
}

function lookupOutputName(idx: number): string {
  return constants.outputNames[idx] ?? `name ${idx}`;
}

// Dynamic EQ filter types (Low Shelf, Bandpass, High Shelf)
const DYNAMIC_EQ_FILTER_TYPES = ['Low Shelf', 'Bandpass', 'High Shelf'];

function lookupDynEqFilter(idx: number): string {
  return DYNAMIC_EQ_FILTER_TYPES[idx] ?? `filter ${idx}`;
}

function rawToDb(raw: number, zeroPoint = 150): string {
  const db = (raw - zeroPoint) / 10;
  return `${db >= 0 ? '+' : ''}${db.toFixed(1)} dB`;
}

function rawToThresholdDb(raw: number): string {
  const db = (raw - 600) / 10;
  return `${db.toFixed(1)} dB`;
}

// ============================================================================
// Format preset data to match Python output exactly
// ============================================================================

type InputChannelOptions = {
  presetWords: number[];
  inputName: string;
  inputGainOffset: number;
  inputMuteOffset: number;
  inputDelayOnOffset: number;
  sectionStart: number;
};

function formatInputChannel(options: InputChannelOptions): string[] {
  const {
    presetWords,
    inputName,
    inputGainOffset,
  ...
  const eqBands = Array.from({length: 9}, (_, i) => {
    const off = bandStart + i * 5;
    return (
      `  EQ Band ${i + 1}: ${lookupFreq(presetWords[off])}, Q=${lookupQ(presetWords[off + 1])}, ` +
      `Gain=${rawToDb(presetWords[off + 2])}, Type=${lookupEqType(presetWords[off + 3])}, ` +
      `Slope=${lookupEqSlope(presetWords[off + 4])}`
    );
  });
  ...
  return [
    ...
    '',
    `  Dynamic EQ: ${presetWords[sectionStart + 4] ? 'ON' : 'OFF'}`,
    ...
    `    Filter: ${lookupDynEqFilter(presetWords[sectionStart + 8])}`,
    `    Slope: ${lookupEqSlope(presetWords[sectionStart + 9])}`,
    '',
    ...eqBands,
    '',
  ];
...
function formatOutputChannel(presetWords: number[], outIdx: number): string[] {
...
/**
 * Extract preset words directly from DCX file data.
 */
...
describe('preset-parser', () => {
  describe('parseDcxFileToStates', () => {
    ...
    it('should parse input and output channels', () => {
      ...
      expect(Object.keys(firstPreset.state.inputs.A.eqs).length).toBe(9);
...
"`;
        ...
        console.log('Python comparison skipped:', (error as Error).message);
      }
    });
