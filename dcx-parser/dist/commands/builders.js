 ...
 */
import { SYSEX_START, SYSEX_END, VENDOR_ID, MODEL_ID, DEFAULT_DEVICE_ID, CMD_DUMP_REQUEST, CMD_RECALL, CMD_STORE, CMD_INIT_SYNC, CMD_WRITE_DATA, CMD_DIRECT, CMD_LISTEN_MODE, PACKET_TYPE_HEADER, PACKET_TYPE_PAGE, HEADER_SIZE, } from '../constants/protocol.js';
 ...
 */
export function buildHeader(deviceId, command) {
 ...
 */
export function buildPingCommand(deviceId = DEFAULT_DEVICE_ID) {
 ...
 */
export function buildPageDumpRequest(page, deviceId = DEFAULT_DEVICE_ID) {
    return new Uint8Array([
 ...
 */
export function buildEditBufferRequest(part, deviceId = DEFAULT_DEVICE_ID) {
    return new Uint8Array([
        ...buildHeader(deviceId, CMD_DUMP_REQUEST),
 ...
 */
export function buildListenModeCommand(mode = 0x0c, state = 0x00, deviceId = DEFAULT_DEVICE_ID) {
    return new Uint8Array([
 ...
 */
export function buildRecallCommand(slot, deviceId = DEFAULT_DEVICE_ID) {
    return new Uint8Array([
 ...
 */
export function buildStoreCommand(slot, deviceId = DEFAULT_DEVICE_ID) {
    return new Uint8Array([
 ...
 */
export function buildSyncCommand(deviceId = DEFAULT_DEVICE_ID) {
 ...
 */
export function buildDataPacket(type, pageNumber, data, deviceId = DEFAULT_DEVICE_ID, bank = 0x00) {
 ...
 */
export function buildHeaderPacket(data, deviceId = DEFAULT_DEVICE_ID) {
    return buildDataPacket(PACKET_TYPE_HEADER, 0, data, deviceId);
}
/**
 * Build a page packet for restore (type 0x0C).
 */
export function buildPagePacket(pageNumber, data, deviceId = DEFAULT_DEVICE_ID) {
    return buildDataPacket(PACKET_TYPE_PAGE, pageNumber, data, deviceId);
}
/**
 * Build a direct parameter command.
 */
export function buildDirectCommand(parameters, deviceId = DEFAULT_DEVICE_ID) {
    const parameterBytes = [];
    for (const { channel, param, value } of parameters) {
        parameterBytes.push(channel & 0x7f, param & 0x7f, Math.floor(value / 128) & 0x7f, value % 128);
    }
    return new Uint8Array([
 ...
 */
function findParameterDefinition(target) {
 ...
 */
function findDirectAddress(target, key) {
    for (const [directKey, def] of directLookup.entries()) {
        if (def.key !== key)
            continue;
        const matchesTarget = (target.kind === 'setup' && def.target.kind === 'setup') ||
            (target.kind === 'channel' &&
                def.target.kind === 'channel' &&
                def.target.group === target.group &&
                def.target.id === target.id) ||
