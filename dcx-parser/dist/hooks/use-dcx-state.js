...
import { dcxStore } from '../store/dcx-store.js';
/**
...
export function useDcxState(connection) {
    ...
    const state = useSyncExternalStore(dcxStore.subscribe, dcxStore.getSnapshot, () => null);
        ...
        const unsubscribe = connection.onMessage((data) => {
            ...
            if (message.type === 'editBuffer') {
                ...
                if (part0Ref.current && part1Ref.current) {
                    ...
                    const combined = new Uint8Array(part0Ref.current.length + part1Ref.current.length);
    ...
    const setSetup = useCallback(async (key, value) => {
        if (!connection)
            return;
        const target = { kind: 'setup', key: String(key) };
