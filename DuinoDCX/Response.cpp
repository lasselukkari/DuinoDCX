...
Response::Response(Client *client, uint8_t *writeBuffer, int writeBufferLength)
    : m_stream(client), m_headers(), m_contentLengthSet(false),
      m_contentTypeSet(false), m_keepAlive(false), m_statusSent(0),
      m_headersSent(false), m_sendingStatus(false), m_sendingHeaders(false),
...
void Response::bypassResponse() {
  // Mark response as already handled by external code (e.g., WebSocket upgrade)
  // This prevents aWOT from sending its own headers/status AND keeps connection
  // open
  m_headersSent = true;
  m_statusSent = 200; // Or any non-zero value to indicate status was sent
  m_ended = true;
  m_responseOpen = true; // Keep connection open for WebSocket
}
